...
public:
	...
	void predictStandardData(){
		...
		predicting(EVALB_OUT_PREDICT_NAME, &standardGrammar);
			...
			while (getline(fscore, s)){
				int p = s.find("Bracketing FMeasure       =");
				if (p < s.size()){
	...
	void training(int times){
		...
		LogDouble lastLikelihood = 0;
		for (int time = 1; time <= times; time++){
			printf("The %d-th iter\n", time);
			...
			int i, j, k, x, y, z;
			#pragma omp parallel for private(i, j, k, x, y, z) schedule(dynamic)
			for (int sen = 0; sen < numSentencesTraining; sen++){
				...
				for (i = 0; i < words[sen].size(); i++)
					for (j = 0; j < words[sen].size(); j++)
						for (k = 0; k < grammar.numNonTerminals; k++){
							alpha[i][j][k] = LOG_ZERO;
							beta[i][j][k] = LOG_ZERO;
						}
				for (j = 0; j < words[sen].size(); j++)
					for (i = 0; i < grammar.numNonTerminals; i++)
						beta[j][j][i] = grammar.LogTerminalProbability[i][grammar.indexTerminal[words[sen][j]]];
				for (j = 0; j < words[sen].size(); j++)
					for (i = j - 1; i >= 0; i--)
						for (k = i; k <= j - 1; k++)
							for (x = 0; x < grammar.numNonTerminals; x++)
								for (y = 0; y < grammar.numNonTerminals; y++)
									for (z = 0; z < grammar.numNonTerminals; z++)
				...
				for (i = 0; i < words[sen].size(); i++)
					for (x = 0; x < grammar.numNonTerminals; x++)
						for (y = 0; y < grammar.numTerminals; y++)
							if (y == grammar.indexTerminal[words[sen][i]])
								countNon[x][y] = LogarithmOfAddition(countNon[x][y], alpha[i][i][x] + grammar.LogTerminalProbability[x][y] - beta[0][words[sen].size() - 1][0]);
			...
			free(count, grammar.numNonTerminals, grammar.numNonTerminals, grammar.numNonTerminals);
			free(countNon,grammar.numNonTerminals, grammar.numTerminals);

			LogDouble likelihoodTest = 0;

			for (int sen = 0; sen < numSentencesTest; sen++){
				LogDouble ***beta;
				malloc(&beta, MAX_WORDS, MAX_WORDS, grammar.numNonTerminals);
				
				for (int i = 0; i < wordsTest[sen].size(); i++)
					for (int j = 0; j < wordsTest[sen].size(); j++)
						for (int k = 0; k < grammar.numNonTerminals; k++)
							beta[i][j][k] = LOG_ZERO;
				for (int j = 0; j < wordsTest[sen].size(); j++)
					for (int i = 0; i < grammar.numNonTerminals; i++)
						beta[j][j][i] = grammar.LogTerminalProbability[i][grammar.indexTerminal[wordsTest[sen][j]]];
				for (int j = 0; j < wordsTest[sen].size(); j++)
					for (int i = j - 1; i >= 0; i--)
						for (int k = i; k <= j - 1; k++)
							for (int x = 0; x < grammar.numNonTerminals; x++)
								for (int y = 0; y < grammar.numNonTerminals; y++)
									for (int z = 0; z < grammar.numNonTerminals; z++)
										beta[i][j][x] = LogarithmOfAddition(beta[i][j][x], grammar.LogNonTerminalProbability[x][y][z] + beta[i][k][y] + beta[k + 1][j][z]);	
				likelihoodTest = likelihoodTest + beta[0][wordsTest[sen].size() - 1][0];
				
				free(beta, MAX_WORDS, MAX_WORDS, grammar.numNonTerminals);
			}
			printf("%.20Lf\t", likelihoodTest.get());
			...
			if (abs((lastLikelihood.get() - likelihood.get()) / likelihood.get()) < 0.0001) break;
			else lastLikelihood = likelihood;
	...
	void predicting(string s, TrainingGrammar *myGrammar){
		...
		for (int i = 0; i < numSentencesTest; i++){
			...
			free(table, MAX_WORDS, MAX_WORDS, myGrammar->numNonTerminals);
			free(record, MAX_WORDS, MAX_WORDS, myGrammar->numNonTerminals);
		}
		evalbOut->close();
	}
