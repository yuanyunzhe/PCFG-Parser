...
public:
	...
	void predictStandardData(){
		...
		for (int i = 0; i < cnf.binaryRules.size(); i++){
			...
			if (standardGrammar.indexNonTerminal.find(rule.nonTerminalParent) == standardGrammar.indexNonTerminal.end())
				standardGrammar.indexNonTerminal[rule.nonTerminalParent] = standardGrammar.indexNonTerminal.size();
			if (standardGrammar.indexNonTerminal.find(rule.nonTerminalLeft) == standardGrammar.indexNonTerminal.end())
				standardGrammar.indexNonTerminal[rule.nonTerminalLeft] = standardGrammar.indexNonTerminal.size();
			if (standardGrammar.indexNonTerminal.find(rule.nonTerminalRight) == standardGrammar.indexNonTerminal.end())
				standardGrammar.indexNonTerminal[rule.nonTerminalRight] = standardGrammar.indexNonTerminal.size();
		}
		for (int i = 0; i < cnf.unaryRules.size(); i++){
			...
			if (standardGrammar.indexNonTerminal.find(rule.nonTerminal) == standardGrammar.indexNonTerminal.end())
				standardGrammar.indexNonTerminal[rule.nonTerminal] = standardGrammar.indexNonTerminal.size();
			if (standardGrammar.indexTerminal.find(rule.terminal) == standardGrammar.indexTerminal.end())
				standardGrammar.indexTerminal[rule.terminal] = standardGrammar.indexTerminal.size();
		}
		...
		for (int i = 0; i < cnf.unaryRules.size(); i++){
			UnaryRule rule = cnf.unaryRules[i];
			standardGrammar.LogTerminalProbability[standardGrammar.indexTerminal[rule.nonTerminal]][standardGrammar.indexTerminal[rule.terminal]] = log(rule.probability);
		}
		malloc(&table, MAX_WORDS, MAX_WORDS, standardGrammar.numNonTerminals);
		malloc(&record, MAX_WORDS, MAX_WORDS, standardGrammar.numNonTerminals);
			...
			for (int i = 0; i < num; i++){
				...
				for (int j = 0; j < wordsTest[i].size(); j++)
					if (grammar.indexTerminal.find(wordsTest[i][j]) == grammar.indexTerminal.end())
						grammar.indexTerminal[wordsTest[i][j]] = grammar.indexTerminal.size();
			}
			// for (int i = 0; i < num; i++){
			// 	wordsTest[i].clear();
			// 	parseTree = new ParseTree;
			// 	parseTree->load(*treeInTest);
			// 	parseTree->preOrderTraversal(parseTree->root, wordsTest[i]);
			// 	for (int j = 0; j < wordsTest[i].size(); j++)
			// 		if (grammar.indexTerminal.find(wordsTest[i][j]) == grammar.indexTerminal.end())
			// 			grammar.indexTerminal[wordsTest[i][j]] = grammar.indexTerminal.size();
			// }
	...
	void training(int times){
		...
		for (int time = 1; time <= times; time++){
			...
			// for (int aaa = 0; aaa < binaryRules.size(); aaa++)
			// 	cout << binaryRules[aaa].nonTerminalParent << " " << binaryRules[aaa].nonTerminalLeft << " " << binaryRules[aaa].nonTerminalRight << " " << binaryRules[aaa].probability << endl;
			// for (int aaa = 0; aaa < unaryRules.size(); aaa++)
			// 	cout << unaryRules[aaa].nonTerminal << " " << unaryRules[aaa].terminal << " " << unaryRules[aaa].probability << endl;
		}
	}
	...
	void calculateInsideOutside(int p){
		...
		// 	for (int i = 0; i <= j; i++){
		// 		if ((i == 0) && (j == words[p].size() - 1)) continue;
		// 		for (int k = 0; k < i; k++)
		// 			for (int x = 0; x < grammar.numNonTerminals; x++)
		// 				for (int y = 0; y < grammar.numNonTerminals; y++)
		// 					for (int z = 0; z < grammar.numNonTerminals; z++)
